...
def write_to_txt(url_file, out_file, makevocab=False):
  print("Making txt file for URLs listed in %s..." % url_file)
  url_list = read_text_file(url_file)
  url_hashes = get_url_hashes(url_list)
  story_fnames = [s+".story" for s in url_hashes]
  num_stories = len(story_fnames)
...
if __name__ == '__main__':
  ...
  write_to_txt(all_test_urls, os.path.join(finished_files_dir, "test.txt"))
  write_to_txt(all_val_urls, os.path.join(finished_files_dir, "val.txt"))
  write_to_txt(all_train_urls, os.path.join(finished_files_dir, "train.txt"), makevocab=True)
